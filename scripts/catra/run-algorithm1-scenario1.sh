...
for STATION_COUNT in ${STATIONS}; do
  for DISTANCE_SET in ${ADJACENT_DISTANCE_SETS}; do
    ...
    ./ns3 run \
      "catra-scenario1 --mode=measure-only --n=${STATION_COUNT} --distances=${DISTANCE_SET} --simTime=${SIM_TIME} --ep=${EP} --run=${RUN} --strict=true --printTopology=false --csv=${THROUGHPUT_CSV} --stationCsv=${STATION_CSV}" \
      --no-build
