#!/usr/bin/env bash
set -euo pipefail

CSV_PATH="${1:-results/scenario1/scenario1-throughput.csv}"
SIM_TIME="${SIM_TIME:-300}"
# Each n=3 item is "n0-n1,n1-n2" in meters. Both adjacent links must be at
# most 250 m so that each forwarding hop is inside the transmission range.
# Override example: ADJACENT_DISTANCE_SETS="180,220" ./scripts/catra/run-scenario1-n3.sh
ADJACENT_DISTANCE_SETS="${ADJACENT_DISTANCE_SETS:-200,250 250,200 200,200}"
TRAFFIC_PROFILE="${TRAFFIC_PROFILE:-paper}"
TRACE_CW="${TRACE_CW:-true}"
VERBOSE_CW="${VERBOSE_CW:-false}"
MAC_HOP_INTERVAL="${MAC_HOP_INTERVAL:-1}"

./ns3 build scenario1 -j 2
for DISTANCE_SET in ${ADJACENT_DISTANCE_SETS}; do
  DISTANCE_TAG="${DISTANCE_SET//,/-}"
  # Use one CSV per distance so results from different geometries cannot be
  # confused. Example suffix: links200-250m means 200 m then 250 m.
  SCENARIO_CSV="${CSV_PATH%.csv}-links${DISTANCE_TAG}m.csv"
  CW_TRACE_CSV="${CSV_PATH%.csv}-cw-links${DISTANCE_TAG}m.csv"
  MAC_HOP_CSV="${CSV_PATH%.csv}-mac-hop-links${DISTANCE_TAG}m.csv"
  ./ns3 run \
    "scenario1 --mode=route-probe --n=3 --distances=${DISTANCE_SET} --strict=true --printTopology=false" \
    --no-build
  ./ns3 run \
    "scenario1 --mode=scenario1 --trafficProfile=${TRAFFIC_PROFILE} --n=3 --distances=${DISTANCE_SET} --simTime=${SIM_TIME} --traceCw=${TRACE_CW} --verboseCw=${VERBOSE_CW} --measureMacHops=true --macHopInterval=${MAC_HOP_INTERVAL} --strict=true --printTopology=false --csv=${SCENARIO_CSV} --cwTraceCsv=${CW_TRACE_CSV} --macHopCsv=${MAC_HOP_CSV}" \
      --no-build
  python3 scripts/catra/plot-scenario1.py "${SCENARIO_CSV}" --traffic-profile "${TRAFFIC_PROFILE}"
done
